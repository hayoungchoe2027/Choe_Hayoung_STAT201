...
max_spread <- which.max(spread)

big_spread <- spread[max_spread]
bear_score <- bears[max_spread]
opponent_score <- opponent[max_spread]

cat("The largest win spread is", paste0(big_spread, ", when the Bears scored"), bear_score, "points and the Opponent scored", opponent_score,"points.")
```
...
```{r}
more_games_bears <- c(37, 9)
more_games_opponent <- c(17, 17)
more_games <- cbind(more_games_bears, more_games_opponent)

game_score <- rbind(game_score, more_games)
print(game_score)
```
...
area <- function(length = 48, width = 48) {
  inches_area <- length * width
  feet_area <- inches_area / 144
  return(c(inches_area, feet_area))
}

output <- area(length = 102, width = 60)
cat("The area in square inches is", paste0(output[1], ".\n"))
cat("The area in square feet is", paste0(output[2], "."))
```
...
```{r}
big_word <- function(string) {
  count <- nchar(string)
  max_count <- max(count)
  return(string[count == max_count])
}
